...
float cameraSpeed = 10.0f; // WASD Movement Speed
...
bool isFirstPotControlActive = false;
glm::mat4 modelWrist;
glm::mat4 objectXScale = glm::mat4(1.0f);
glm::mat4 objectXform = glm::scale(glm::translate(glm::mat4(1.0f), glm::vec3(0.5f, 0.0f, 0.0f)), glm::vec3(0.08f, 0.08f, 0.08f));
glm::mat4 lastPotTransform = objectXform;
glm::mat4 wristInitialTransform = glm::mat4(1.0f);
...
void myDisplay()
{
	...
	if (PotControl)
	{
		objectXScale = glm::mat4(1.0f);
		glm::vec3 potInitialPosition(0.5f, 0.0f, 0.0f);
		objectXScale = glm::translate(objectXScale, potInitialPosition); // Pot 초기 위치로 이동

		// PotControl이 처음 활성화될 때 Wrist의 위치와 회전 저장 
		if (!isFirstPotControlActive)
		{
			wristInitialTransform = modelWrist;
			isFirstPotControlActive = true;
		}
		glm::mat4 wristRelativeTransform = wristInitialTransform;  // 초기 상태 유지

		// Wrist의 현재 변환을 Pot에 적용 (Wrist 기준)
		objectXform = modelWrist * glm::inverse(wristRelativeTransform) * objectXScale;
		objectXform = glm::scale(objectXform, glm::vec3(0.08f, 0.08f, 0.08f));

		lastPotTransform = objectXform;
	}
	else
	{
		isFirstPotControlActive = false;
		objectXform = lastPotTransform;
	}
...
int main()
{
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		PhongShader->setBool("lKey", lKeyPressd);
		...
		FloorShader->setBool("lKey", lKeyPressd);
...
void mouse_callback(GLFWwindow* window, double xpos, double ypos)
{
	...
	if (RightButtonDown)
	{
		camera.ProcessMouseMovement(xoffset * 90, yoffset * 90);
	}
}
